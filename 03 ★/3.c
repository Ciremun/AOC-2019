...
{
    if (!found_intersection && x != START && y != START)
    {
        printf("fount first intersect (%d, %d) in %d steps\n", x, y, steps);
...
void part_1(char lines[LINES_COUNT][LINE_LENGTH])
{
    ...
    for (int cursor = 0; lines[1][cursor] != '\0'; cursor++)
    {
        ...
        {
        case UP:
            corner = (prev_op == LEFT || prev_op == RIGHT);
            if (corner)
            {
                steps--;
            }
        ...
        case DOWN:
            ...
            break;
        case LEFT:
            corner = (prev_op == UP || prev_op == DOWN);
            if (corner)
            {
                steps--;
            }
    ...
    // bool(*occupied_2)[GRID_SIZE_Y] = malloc(sizeof(bool) * GRID_SIZE_X * GRID_SIZE_Y);
    // op_count = 0;
    // fill_2d_arr(path, MAX_OP_COUNT, START);
    // intersect_count = 0;
    // fill_2d_arr(path_2, MAX_OP_COUNT, START);
